...
printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Refused: ScheduleWakeup only works in /loop dynamic mode. The fuwafuwa heartbeat invokes claude -p (one-shot print mode), so this silently no-ops and the session exits without continuation — orphaning brain/session.lock. If you need to wait, poll inline with sleep+check, or just exit cleanly via session end and let the systemd timer re-fire in 2min."}}\n'
